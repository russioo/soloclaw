...
export async function generateThought(data: CycleData): Promise<string> {
  const client = getClient();
  if (!client) return fallbackThought(data);
  ...
  try {
    ...
    if (thought && thought.length > 5) return thought;
  } catch (err) {
...
function fallbackThought(data: CycleData): string {
  if (data.skipped) return "Scanning the vault. Fees are building up.";
